    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[85vh]">
        ...
        <div className="p-6 bg-gray-50/50 border-b border-gray-100 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2 hover:bg-white rounded-full transition-all text-gray-400 hover:text-black shadow-sm">
            <X size={20} />
          </button>
        </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-gray-800 line-clamp-1 flex items-center gap-2">
                                        {song.tieuDe}
                                        {song.trangThai === 'hidden' && <span className="text-[10px] bg-red-50 text-red-500 px-1.5 py-0.5 rounded border border-red-100 uppercase font-bold">Đã ẩn</span>}
                                    </p>
                                    <p className="text-[11px] text-gray-400 font-medium tracking-wide">Thời lượng: {Math.floor(song.thoiLuongGiay / 60)}:{(song.thoiLuongGiay % 60).toString().padStart(2, '0')} • Nghệ sĩ: {song.tenNgheSi}</p>
                                </div>
                                </div>
                            </div>
